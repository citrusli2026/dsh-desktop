...
if (typeof runtime.version !== 'string' || !/^v24\.\d+\.\d+$/.test(runtime.version)) failures.push('bundled Node pin must be a Node 24 version')
if (!/^https:\/\/nodejs\.org\/dist\/v24\.\d+\.\d+\/SHASUMS256\.txt$/.test(runtime.source)) failures.push('bundled Node pin must come from the recorded official Node.js 24.x checksum source')
