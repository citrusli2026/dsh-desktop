...
(function () {
  ...
  var I18N = {
    zh: {
      ...
      'vision.marker': '扩展功能 · 视觉',
      'vision.title': '粘贴图片，纯文本模型也能「看见」',
      'vision.body': '内置 ModLens 视觉插件随安装包预装、开机自动挂载。在对话里粘贴截图或照片，agent 就会自动调用识别工具输出 OCR、布局与语义结构——专为没有原生视觉的模型准备。',
      'vision.steps': '<li>首次粘贴图片 → 三步向导：复用本机引擎登录或添加免费 Gemini Key</li><li>向导用内置示例图做一次真实识别测试，成功即完成配置</li><li>扩展 → Vision (ModLens) → Settings… 随时改引擎、重测或查看本地诊断</li>',
      'vision.note': '引擎凭据只保存在 ~/.modlens/config.json（mode 600）；诊断完全本地运行，不联网、不耗配额。',
      ...
      'ft.p7': '<h3>粘贴即识图,视觉开箱即用</h3><p>内置 ModLens 视觉插件,粘贴截图或照片后 agent 自动调用 <code>modlens_read_image</code> 输出 OCR、布局与语义结构。首次粘贴有三步向导,支持 Gemini、任意 OpenAI 兼容端点、本地 CLI 登录复用等引擎。</p>',
      'theme.toggle': '切换明暗主题',
      ...
      'faq.q9': '图片识别(ModLens)需要什么引擎?',
      'faq.a9': '内置的 ModLens 视觉插件支持多种引擎:Gemini API(可免费申请)、任意 OpenAI 兼容端点、Anthropic、Antigravity CLI、Claude Code CLI、Kimi Code CLI,或复用你本机 Claude Code / Codex / OpenCode / Pi / Grok 的登录。首次粘贴图片会弹出三步向导;凭据只保存在 <code>~/.modlens/config.json</code>(mode 600),识别失败时设置窗口可一键运行本地诊断,查看每个引擎的状态与修复建议。',
    ...
    en: {
      ...
      'vision.marker': 'EXTENSION · VISION',
      'vision.title': 'Paste an image. Text-only models can now "see".',
      'vision.body': 'The bundled ModLens vision plugin ships with every installer and mounts on every boot. Paste a screenshot or photo into chat and the agent calls the recognition tool for OCR, layout, and semantic structure — built for models without native vision.',
      'vision.steps': '<li>First paste → three-step wizard: reuse a local engine login or add a free Gemini key</li><li>The wizard runs a real recognition test on a bundled sample image to confirm</li><li>Extensions → Vision (ModLens) → Settings… changes engines, retests, or runs the local diagnosis anytime</li>',
      'vision.note': 'Engine credentials live only in ~/.modlens/config.json (mode 600); diagnosis is fully local — no network, no quota.',
      ...
      'ft.p7': '<h3>Paste-to-see vision, out of the box</h3><p>The bundled ModLens plugin lets the agent call <code>modlens_read_image</code> for OCR, layout, and semantics whenever you paste an image. A three-step first-run wizard covers Gemini, any OpenAI-compatible endpoint, and reuse of local CLI logins.</p>',
      ...
      'faq.q9': 'What engine does image recognition (ModLens) need?',
      'faq.a9': 'The bundled ModLens plugin supports Gemini API (free key available), any OpenAI-compatible endpoint, Anthropic, Antigravity CLI, Claude Code CLI, and Kimi Code CLI — or it can reuse your existing Claude Code / Codex / OpenCode / Pi / Grok logins. The first image paste opens a three-step wizard. Credentials live only in <code>~/.modlens/config.json</code> (mode 600), and a one-click local diagnosis in the settings window explains per-engine status and fixes.',
