...
export var I18N = {
  zh: {
    ...
    'a11y.menuOpen': '打开导航', 'a11y.menuClose': '关闭导航',
      'brand.community': '社区版',
    'nav.workflow': '产品定位', 'nav.showcase': '实拍展示', 'nav.features': '核心能力', 'nav.version': '版本信息', 'nav.faq': '帮助支持', 'nav.why': '为何桌面版', 'nav.install': '安装指南', 'nav.cta': '立即下载',
    'hero.h1': 'DeepSeek Harness<br />桌面版，可靠的 <em>本地桌面应用</em>。',
    'hero.sub': '社区维护的 DeepSeek Harness 桌面版：开箱即用。自带运行时、独立数据目录、桌面入口和恢复路径；只增强桌面体验，不改写 Harness 行为。',
    'hero.cta': '下载桌面版',
    'hero.ctaMobile': '在电脑上下载',
    'hero.secondary': '它和命令行有什么不同 →',
    'hero.meta': '无需 Node.js',
    'hero.preview': '本地运行时已就绪',
    'hero.downloads': '↓{n}次下载 · GitHub + GitCode',
    'hero.highlightPlugin.label': '插件市场', 'hero.highlightPlugin.value': '数千插件，按需安装', 'hero.highlightPlugin.note': '不进安装包，先装市场再由你选择。',
    'hero.highlightBalance.label': '账户余额', 'hero.highlightBalance.value': '托盘里直接查看', 'hero.highlightBalance.note': '有可用 API Key 时，在托盘与桌面设置中显示。',
    'hero.highlightCore.note': '内置内核，更新失败可回到稳定版本。',
    'trust.local': 'Loopback 本地运行', 'trust.isolated': '默认独立数据目录',
    'trust.guarded': '沙箱隔离，权限默认最小', 'trust.verifiable': '公开 CI 与真实应用测试',
    'wf.marker': '产品定位', 'wf.title': '官方 Harness，原样跑进桌面窗口',
    'wf.lead': '桌面壳负责安装、启动、恢复和入口；Agent、会话、工具、模型仍由官方 Harness 负责。',
    'wf.s1.title': '开箱即用', 'wf.s1.body': '内置 Node.js 与依赖，安装后直接启动。',
    'wf.s2.title': '本地且隔离', 'wf.s2.body': '只在本机通信，不暴露到局域网；数据默认放在独立的 ~/.dsh-desktop。',
    'wf.s3.title': '出错可恢复', 'wf.s3.body': '安全模式隔离坏插件；错误页可一键升级或禁用可疑插件。',
    'wf.adds.label': '桌面壳负责', 'wf.keeps.label': 'Harness 继续负责',
    'wf.adds': '<li>窗口、托盘、快捷键与通知</li><li>运行时封装、进程守护与更新</li><li>安全模式、诊断与权限边界</li>',
    'wf.keeps': '<li>Agent、会话、工具与模型</li><li>官方 WebUI 与插件行为</li><li>账户配置与官方设置</li>',
    'ext.marker': '扩展功能',
    'ext.title': '扫码，把 Harness 带到手机或平板',
    'ext.body': '从「桌面工具」启动局域网 Web 代理，手机与电脑同网后扫码即可进入 Harness。',
    'ext.note': '不需要单独的 Android / iOS 应用。',
    'notify.marker': '桌面通知',
    'notify.title': '任务完成或需要你确认时，通知你一声',
    'notify.body': '窗口未聚焦或收进托盘时，通知会话完成、失败、停止或等待确认。点击即可唤回工作区。',
    'notify.steps': '<li>窗口未聚焦或隐藏在托盘时弹出系统通知</li><li>点击通知，直接显示并聚焦 dsh-desktop 窗口</li><li>在设置 → 桌面设置中随时关闭；配置只保存在本机</li>',
    'notify.note': '不读取屏幕、不做视觉识别、不上传数据；首次使用只记录当前状态，不会在启动时乱弹通知。',
    'safe.marker': '故障自愈',
    'safe.title': '插件装坏了？安全模式帮你隔离，再精准定位',
    'safe.body': '第三方插件出问题时，安全模式只运行官方与内置扩展，并指出疑似插件。',
    'safe.steps': '<li>错误页「以安全模式启动」，或从「桌面工具 / 桌面设置」进入</li><li>官方与内置扩展照常运行，第三方插件被隔离</li><li>按横幅或诊断报告里的疑似插件，到官方「设置 → 插件」卸载</li>',
    ...
    'latest.marker': '当前版本重点', 'latest.title': '这个版本：删掉的东西，都能找回',
    'latest.lead': '新增桌面垃圾桶：会话、预设、插件与 Agent 删除的文件，都先进垃圾桶，随时还原。',
    'latest.items': '<li><b>首次成功：</b>主界面四步引导在首条任务完成后自动结束。</li><li><b>垃圾桶兜底：</b>误删的会话在「设置 → 垃圾桶」一键还原。</li><li><b>隐私可控：</b>联网检查需主动勾选，社区插件仍完全可选。</li>',
    ...
    'ft.marker': '桌面能力', 'ft.title': '稳定的桌面入口，该有的能力都在',
    ...
    'ft.group3.title': '恢复与边界', 'ft.group3.body': '出问题时给你明确的下一步；权限和数据始终留在本机。',
    'ft.p1': '<h3>无需安装 Node.js</h3><p>内置 Node.js 与锁定依赖，下载安装包即可启动。</p>',
    'ft.p2': '<h3>独立数据目录，环境隔离</h3><p>默认使用 <code>~/.dsh-desktop</code>，不影响命令行；需要共享时再设置 <code>DSH_HOME</code>。</p>',
    'ft.p4': '<h3>持续守护与可靠更新</h3><p>崩溃按预算重启，保留托盘入口；更新失败会回到可用内核。</p>',
    'ft.p5': '<h3>渲染层能力默认收敛</h3><p>沙箱、上下文隔离和权限拒绝默认开启，桌面通知不读取屏幕。</p>',
    'ft.p6': '<h3>问题发生时，可检查并恢复</h3><p>日志和诊断报告在本地生成，自动遮罩常见密钥，不会自动上传。</p>',
    'ft.p7': '<h3>不用找窗口，快捷键直接唤回</h3><p>默认 <code>Ctrl/Cmd + Shift + Space</code>，冲突时仍可用托盘和右键。</p>',
    'nav.market': '插件市场',
    'mk.marker': '社区插件', 'mk.title': '数千个社区插件，全部由你选择',
    'mk.lead': 'dsh-desktop 只提供清晰入口和故障恢复支持；社区插件不属于安装包，也不会在首次启动时自动出现。',
    'mk.route1.title': '浏览与一键安装',
    'mk.route1.body': '在 Harness「设置 → 插件市场」按分类浏览、安装和更新。',
    'mk.route2.title': '装坏了也不怕',
    'mk.route2.body': '插件装坏时，错误页可直接升级到最新版或先禁用；安全模式随时兜底。',
    'mk.note': '如何安装：按需在「设置 → 桌面设置 → 插件市场」手动安装 dsh-market，再到 Harness「设置 → 插件市场」手动选择其他插件；首次任务不依赖插件市场。',
    'theme.toggle': '切换明暗主题',
    'vr.marker': '版本信息', 'vr.title': '版本号怎么读',
    'vr.core': '<b>内核版本</b> —— 内置的 <code>@deepseek-ai/dsh</code> 版本，跟随官方更新，新版本会尽快跟进。',
    'vr.shell': '<b>桌面版修订号</b> —— 同一内核下，桌面版自身（窗口、更新、修复）的更新次数。',
    ...
    'faq.a1': '当前 macOS 安装包尚未签名公证，首次启动请 <b>右键 → 打开</b>。如果仍没有放行选项，且你确认安装包来源可信，可在终端执行：<br><code class="faq-command">xattr -dr com.apple.quarantine "/Applications/dsh-desktop.app"</code><code class="faq-command">open "/Applications/dsh-desktop.app"</code>这会移除下载隔离标记，但不会添加 Apple 签名或公证。<br><br>安装包由公开 CI 构建，可用来源证明验证真伪：<br><code class="faq-command">gh attestation verify /path/to/dsh-desktop-{ver}-arm64-mac.dmg -R citrusli2026/dsh-desktop</code>',
    'faq.q2': 'Windows SmartScreen 拦截怎么办？',
    'faq.a2': '当前 Windows 安装包尚未做代码签名。点击 <b>“更多信息” → “仍要运行”</b> 即可。安装包由 GitHub Actions 从公开源码构建，可全程审计；也可用以下命令验证来源：<br><code class="faq-command">gh attestation verify /path/to/dsh-desktop-setup-{ver}.exe -R citrusli2026/dsh-desktop</code>',
    'faq.q3': '这和 DeepSeek 官方是什么关系？',
    'faq.a3': '社区维护的桌面封装：不改写 Harness 行为，数据与配置保持独立。项目与 DeepSeek AI 无隶属、授权或合作关系；DeepSeek Harness 是 DeepSeek 的商标，本项目仅在 MIT 许可下再打包 <a href="https://www.npmjs.com/package/@deepseek-ai/dsh" target="_blank" rel="noopener">@deepseek-ai/dsh</a>。',
    'faq.q4': '我在用 npx @deepseek-ai/dsh web，配置会带过来吗？',
    'faq.a4': '不会——桌面版是独立环境，数据放在 <code>~/.dsh-desktop</code>，与 CLI 的 <code>~/.dsh</code> 互不影响。想沿用 CLI 配置，启动前设 <code>DSH_HOME=~/.dsh</code> 即可。',
    ...
    'faq.a6': '不会。当前功能不需要这些 Electron Web 权限，桌面版默认拒绝摄像头、定位、网页通知、屏幕采集和文件系统这类请求；桌面通知是可关闭的本地功能，不读取屏幕，也不上传数据。',
    ...
    'faq.a10': 'dsh-desktop 安装包不内置社区插件，所有插件都需要用户手动安装。需要时在 <b>设置 → 桌面设置 → 插件市场</b> 手动安装 dsh-market；装好后打开 Harness「设置 → 插件市场」，即可浏览并一键安装数千个社区插件与主题。首次任务不需要插件市场；安全模式会隔离所有第三方插件。',
    ...
    'faq.a12': '可以。在 dsh-desktop 的「设置 → 桌面设置」中点击「重新设置」，按下包含修饰键的组合键（例如 <code>Ctrl + Alt + K</code>）。如果组合键已被其他程序占用，页面会提示更换；开机启动默认关闭，Windows/macOS 可分别选择是否启动后隐藏到托盘。Linux 当前不提供开机启动开关。',
    'faq.q13': '桌面通知会读取屏幕或上传数据吗？',
    'faq.a13': '不会。通知只使用 Harness 已公开的会话运行态和后台任务状态，不读取截图、也不会上传数据。它只在窗口未聚焦时提示完成、失败或需要确认，可在「设置 → 桌面设置」关闭。另外有一个<b>默认关闭</b>的「屏幕捕获」：开启后 Agent 才能用 screen_capture 工具截取屏幕，且截图总是作为附件进入会话、发给你选择的视觉模型；不开启就完全没有这个能力。',
    'faq.q14': '启动失败或插件装坏了怎么办？',
    'faq.a14': '应用仍可打开时，先到「设置 → 桌面设置 → 运行体检」查看本地运行时、目录、loopback、Profile 与插件状态；联网检查需单独勾选，结果不上传也不自动修复。无法启动时，错误页提供「再次启动」「以安全模式启动」「打开日志目录」「导出诊断报告」；检测到可疑插件时还会列出每个插件，可直接「升级」到最新版或「禁用」后重启——禁用只从启动列表移除，不删除文件，可随时在「设置 → 插件」恢复。',
    'faq.q15': '误删的会话或文件还能找回吗？',
    'faq.a15': '可以。桌面版自带垃圾桶：删除的会话、预设和插件都会先进入垃圾桶，在 <b>「设置 → 垃圾桶」</b> 中随时还原。Agent 执行的删除类命令也会被拦截：目标先移入垃圾桶，可疑的删除会被直接拦截并说明原因。垃圾桶保留 30 天，过期自动清理。',
  ...
  en: {
    ...
    'nav.workflow': 'SCOPE', 'nav.showcase': 'SHOWCASE', 'nav.features': 'CAPABILITIES', 'nav.version': 'VERSION', 'nav.faq': 'SUPPORT', 'nav.why': 'WHY DESKTOP', 'nav.install': 'INSTALL GUIDE', 'nav.cta': 'Download',
    'hero.h1': 'DeepSeek Harness<br />as a dependable <em>local desktop app</em>.',
    'hero.sub': 'A community-maintained DeepSeek Harness desktop app: out of the box, with a bundled runtime, isolated data, desktop entry points, and recovery paths — without changing how Harness works.',
    'hero.cta': 'Download for Desktop',
    'hero.ctaMobile': 'Download on your computer',
    'hero.secondary': 'How it differs from the CLI →',
    'hero.meta': 'NO NODE.JS REQUIRED',
    'hero.preview': 'Local runtime ready',
    'hero.downloads': '↓{n} downloads · GitHub + GitCode',
    'hero.highlightPlugin.label': 'PLUGIN MARKET', 'hero.highlightPlugin.value': 'Thousands of plugins', 'hero.highlightPlugin.note': 'Nothing is bundled; install the market and choose each plugin.',
    'hero.highlightBalance.label': 'ACCOUNT', 'hero.highlightBalance.value': 'Balance at a glance', 'hero.highlightBalance.note': 'Shown in the tray and Desktop settings when an API key is available.',
    'hero.highlightCore.note': 'Bundled core with a checked update and rollback path.',
    ...
    'wf.marker': 'SCOPE', 'wf.title': 'The official Harness, unchanged, in a desktop window.',
    'wf.lead': 'The shell handles installation, startup, recovery, and entry points. Official Harness still owns agents, sessions, tools, and models.',
    'wf.s1.title': 'Ready out of the box', 'wf.s1.body': 'Bundled Node.js and dependencies. Install and launch.',
    'wf.s2.title': 'Local and isolated', 'wf.s2.body': 'Loopback-only, with a separate ~/.dsh-desktop by default.',
    'wf.s3.title': 'Recoverable by design', 'wf.s3.body': 'Safe Mode quarantines bad plugins; the error page can update or disable suspects in one click.',
    'wf.adds.label': 'THE DESKTOP SHELL OWNS', 'wf.keeps.label': 'HARNESS STILL OWNS',
    'wf.adds': '<li>Window, tray, shortcut, and notifications</li><li>Runtime packaging, supervision, and updates</li><li>Safe Mode, diagnostics, and permission boundaries</li>',
    'wf.keeps': '<li>Agents, sessions, tools, and models</li><li>Official WebUI and plugin behavior</li><li>Account configuration and official settings</li>',
    'ext.marker': 'EXTENSION',
    'ext.title': 'Scan once. Bring Harness to a phone or tablet.',
    'ext.body': 'Start a LAN Web proxy from Desktop tools. Put the phone and computer on the same network, then scan to enter Harness.',
    'ext.note': 'No separate Android or iOS app required.',
    'notify.marker': 'DESKTOP NOTIFICATIONS',
    'notify.title': 'Finished, failed, or waiting on you — you will know.',
    'notify.body': 'When the window is unfocused or in the tray, notices report completion, failure, stopping, or a request for confirmation. Click one to return.',
    'notify.steps': '<li>System notices appear while the window is unfocused or hidden in the tray</li><li>Click a notice to show and focus the dsh-desktop window</li><li>Turn them off any time in Settings → Desktop settings; preferences stay on this device</li>',
    'notify.note': 'Notices never read the screen, never do visual recognition, and never upload data; the first report only records the current state, so nothing fires at startup.',
    'safe.marker': 'Self-recovery',
    'safe.title': 'A broken plugin? Safe Mode quarantines it and names the culprit',
    'safe.body': 'When a third-party plugin causes trouble, Safe Mode runs only official and built-in extensions and names the likely suspect.',
    'safe.steps': '<li>"Start in Safe Mode" on the error page, or enter it from Desktop tools / Desktop settings</li><li>Official and built-in extensions keep running; third-party plugins are quarantined</li><li>Uninstall the suspected plugin named in the banner or diagnostic report via Settings → Plugins</li>',
    ...
    'dl.total.note': 'Includes all past releases',
    'dl.platformTotal': '↓ {n}',
    ...
    'latest.marker': 'CURRENT RELEASE FOCUS', 'latest.title': 'This release: everything you delete can come back',
    'latest.lead': 'New desktop trash: sessions, presets, plugins, and agent-deleted files land in the trash first, ready to restore.',
    'latest.items': '<li><b>First success:</b> a four-step main-screen guide retires after the first completed task.</li><li><b>One-click health:</b> restore deleted sessions with one click in Settings → Trash.</li><li><b>Private by default:</b> network probes are opt-in and community plugins stay optional.</li>',
    ...
    'ft.marker': 'CAPABILITIES', 'ft.title': 'A dependable desktop entry point with the capabilities that matter',
    ...
    'ft.group3.title': 'Recovery and boundaries', 'ft.group3.body': 'When something fails, you get a clear next action, and your permissions and data stay on this machine.',
    'ft.p1': '<h3>No Node.js install required</h3><p>Node.js and all required dependencies are bundled; install and launch.</p>',
    'ft.p2': '<h3>Isolated data home</h3><p><code>~/.dsh-desktop</code> keeps desktop data separate from the CLI; set <code>DSH_HOME</code> only when you want to share.</p>',
    'ft.p4': '<h3>Continuous supervision and reliable updates</h3><p>Crashes restart automatically with a retry limit, the tray stays available, and failed kernel updates roll back.</p>',
    'ft.p5': '<h3>Renderer capabilities stay constrained</h3><p>Sandboxing, context isolation, and denied permissions are defaults; desktop notices never read the screen.</p>',
    'ft.p6': '<h3>Inspect and recover when something breaks</h3><p>Local logs and diagnostic reports mask common secrets and are never uploaded automatically.</p>',
    'ft.p7': '<h3>Return without hunting for the window</h3><p><code>Ctrl/Cmd + Shift + Space</code> summons the app; the tray and context menu remain safe fallbacks.</p>',
    ...
    'mk.route1.title': 'Browse and one-click install',
    'mk.route1.body': 'Use Harness Settings → Plugin Market to browse, install, and update.',
    'mk.route2.title': 'Safe to experiment',
    'mk.route2.body': 'If a plugin breaks the app, update it to the latest or disable it right from the error page; Safe Mode always has your back.',
    'mk.note': 'How to install: when needed, manually install dsh-market from Settings → Desktop settings → Plugin market, then choose other plugins from Harness Settings → Plugin Market. The first task never depends on the market.',
    'theme.toggle': 'Toggle light/dark theme',
    'vr.marker': 'VERSIONING', 'vr.title': 'Reading the version',
    'vr.core': '<b>Kernel version</b> — the bundled <code>@deepseek-ai/dsh</code> release; new official versions are tracked and land quickly.',
    'vr.shell': '<b>Shell revision</b> — how many times the desktop app itself (window, updates, fixes) has been revised on this kernel.',
    ...
    'faq.a6': 'No. Current features need none of those Electron web permissions, so media, location, Web notifications, capture, and filesystem requests are denied by default. Desktop status notices are an optional local feature; they do not read the screen or upload data.',
    ...
    'faq.a10': 'Community plugins are not bundled or auto-installed in dsh-desktop; every plugin is installed manually by the user. When needed, install dsh-market from <b>Settings → Desktop settings → Plugin market</b>. Then open Harness <b>Settings → Plugin Market</b> to browse and one-click-install community plugins and themes. The first task does not require the market; Safe Mode quarantines all third-party plugins.',
    ...
    'faq.a12': 'Yes. In dsh-desktop, open Settings → Desktop settings, choose Change shortcut, and press a combination with a modifier such as <code>Ctrl + Alt + K</code>. If another app already owns it, the setting reports a conflict. Launch at login is off by default; Windows and macOS can also start hidden in the tray. Linux does not currently expose a launch-at-login switch.',
    'faq.q13': 'Do desktop notices read my screen or upload data?',
    'faq.a13': 'No. Notices use only Harness public session and background-job state. They do not read your screen or upload data. They appear only while the window is unfocused and can be disabled in Settings → Desktop settings. There is also a <b>default-off</b> Screen capture: only when you enable it can the agent use the screen_capture tool, and screenshots always enter the conversation as attachments sent to the vision model you selected.',
    'faq.q14': 'What if startup fails or a plugin breaks the app?',
    'faq.a14': 'If the app still opens, first use Settings → Desktop settings → Run health check for local runtime, folder, loopback, profile, and plugin state. Network probes require a separate opt-in; results are never uploaded or repaired automatically. If startup fails, the error page offers Retry, Start in Safe Mode, Open logs, and Export diagnostics. When a suspect plugin is detected, each one is listed with Update (pull the latest version) and Disable (remove it from the boot list and restart) actions — disabling keeps the files and is reversible from Settings → Plugins.',
    'faq.q15': 'Can I recover deleted sessions or files?',
    'faq.a15': 'Yes. The desktop trash keeps deleted sessions, presets, and plugins recoverable from <b>Settings → Trash</b>. Agent deletion commands are intercepted too: targets move into the trash first, and ambiguous deletions are blocked with an explanation. Entries are kept for 30 days, then cleaned up automatically.',
