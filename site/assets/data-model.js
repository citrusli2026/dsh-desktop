...
export var I18N = {
  zh: {
    ...
    'faq.q10': '安装的插件让 Harness 起不来，怎么办？',
    'faq.a10': '在错误页点击 <b>「以安全模式启动」</b>（或从扩展入口 / 扩展设置进入安全模式）。第三方插件会被隔离，官方与内置扩展照常运行；安全模式横幅与诊断报告会点名疑似插件，到官方「设置 → 插件」卸载它，然后点「退出安全模式」即可。',
    'faq.q11': '可以把 Agent 预设分享给同事或另一台电脑吗？',
    'faq.a11': '可以。在 <b>设置 → 扩展设置 → Agent 预设</b> 选中预设后「导出预设」，得到单个 <code>.dshpreset</code> 文件；对方从同一面板「导入预设」。导入会先检查重名（跳过 / 替换 / 克隆），且只会安装为用户预设，绝不覆盖内置预设。',
    'faq.q10': 'GitHub 下载慢或连不上怎么办？',
    'faq.a10': '国内网络可优先使用 <a href="https://gitcode.com/citrusli2026/dsh-electron-shell/releases" target="_blank" rel="noopener">GitCode 镜像</a>（中文界面自动排在 GitHub 前面；镜像不可用时自动只显示 GitHub）。如仍需走 GitHub，可在下载链接前加社区代理前缀，例如：<br><code class="faq-command">https://ghproxy.net/https://github.com/citrusli2026/dsh-electron-shell/releases/download/…</code>公开代理（ghproxy.net / gh-proxy.com / ghfast.top）均为第三方免费服务，可用性不保证，一个不行就换另一个。',
    'faq.q11': '可以修改唤起快捷键或设置开机启动吗？',
    'faq.a11': '可以。在 dsh-desktop 的「设置 → 扩展设置」中点击「重新设置」，按下包含修饰键的组合键（例如 <code>Ctrl + Alt + K</code>）。如果组合键已被其他程序占用，页面会提示更换；开机启动默认关闭，Windows/macOS 可分别选择是否启动后隐藏到托盘。Linux 当前不提供开机启动开关。',
    'faq.q12': '桌面通知会读取屏幕或上传数据吗？',
    'faq.a12': '不会。通知只使用 Harness 已公开的会话运行态和后台任务状态，不读取截图、不做视觉识别，也不会上传数据。它只在窗口未聚焦时提示完成、失败或需要确认，可在「设置 → 扩展设置」关闭。',
  ...
  en: {
    ...
    'faq.q10': 'A plugin I installed stops Harness from starting — what now?',
    'faq.a10': 'Click <b>“Start in Safe Mode”</b> on the error page (or enter it from the ⋮ panel / Extensions settings). Third-party plugins are quarantined while official and built-in extensions keep running; the Safe Mode banner and diagnostic report name the suspected plugin, which you can uninstall from the official Settings → Plugins manager, then click “Exit Safe Mode”.',
    'faq.q11': 'Can I share an agent preset with a teammate or another computer?',
    'faq.a11': 'Yes. In <b>Settings → Extensions → Agent presets</b>, select a preset and “Export preset” to get a single <code>.dshpreset</code> file; the other side imports it from the same panel. Imports check for a name conflict (skip / replace / clone) and only install as a user preset — never replacing built-in ones.',
    'faq.q10': 'GitHub is slow or unreachable — is there a mirror?',
    'faq.a10': 'Prefer the <a href="https://gitcode.com/citrusli2026/dsh-electron-shell/releases" target="_blank" rel="noopener">GitCode mirror</a> (listed first in the Chinese interface when verified; it disappears automatically when unavailable). Community proxy prefixes such as <code>https://ghproxy.net/</code> in front of the download URL can help for GitHub itself, but they are third-party, free, and unaffiliated — availability is not guaranteed; try another if one fails.',
    'faq.q11': 'Can I change the summon shortcut or launch at login?',
    'faq.a11': 'Yes. In dsh-desktop, open Settings → Extensions, choose Change shortcut, and press a combination with a modifier such as <code>Ctrl + Alt + K</code>. If another app already owns it, the setting reports a conflict. Launch at login is off by default; Windows and macOS can also start hidden in the tray. Linux does not currently expose a launch-at-login switch.',
    'faq.q12': 'Do desktop notices read my screen or upload data?',
    'faq.a12': 'No. Notices use only Harness public session and background-job state. They do not read screenshots, perform visual recognition, or upload data. They appear only while the window is unfocused and can be disabled in Settings → Extensions.',
