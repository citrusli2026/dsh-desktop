...
export var I18N = {
  zh: {
    ...
    'ft.p8': '<h3>桌面习惯由你决定</h3><p>在 Harness 设置 → 通用中录入自己的唤起快捷键，选择是否开机启动、启动后隐藏，以及是否接收任务状态通知。配置只保存在本机；通知只在窗口未聚焦时提示。</p>',
    'theme.toggle': '切换明暗主题',
    ...
    'faq.a11': '可以。在 dsh-desktop 的「设置 → 通用」中点击「重新设置」，按下包含修饰键的组合键（例如 <code>Ctrl + Alt + K</code>）。如果组合键已被其他程序占用，页面会提示更换；开机启动默认关闭，Windows/macOS 可分别选择是否启动后隐藏到托盘。Linux 当前不提供开机启动开关。',
    'faq.q12': '桌面通知会读取屏幕或上传数据吗？',
    'faq.a12': '不会。通知只使用 Harness 已公开的会话运行态和后台任务状态，不读取截图、不做视觉识别，也不会上传数据。它只在窗口未聚焦时提示完成、失败或需要确认，可在「设置 → 通用」关闭。',
  ...
  en: {
    ...
    'ft.p8': '<h3>Your desktop habits, your choice</h3><p>In Harness Settings → General, record a summon shortcut, choose whether to launch at login and start hidden, and control desktop status notices. Preferences stay on this device; notices appear only while the window is unfocused.</p>',
    ...
    'faq.a11': 'Yes. In dsh-desktop, open Settings → General, choose Change shortcut, and press a combination with a modifier such as <code>Ctrl + Alt + K</code>. If another app already owns it, the setting reports a conflict. Launch at login is off by default; Windows and macOS can also start hidden in the tray. Linux does not currently expose a launch-at-login switch.',
    'faq.q12': 'Do desktop notices read my screen or upload data?',
    'faq.a12': 'No. Notices use only Harness public session and background-job state. They do not read screenshots, perform visual recognition, or upload data. They appear only while the window is unfocused and can be disabled in Settings → General.',
