...
export var I18N = {
  zh: {
    ...
    'showcase.s3.title': '扩展设置，把桌面习惯放在一起',
    'showcase.s3.body': '新增的扩展设置把本机数据、局域网配对、快捷键、启动行为、通知、屏幕捕获与恢复工具集中起来；社区插件仍然坚持手动安装。',
    'showcase.s3.items': '<li>完整覆盖配对、快捷键、开机启动、托盘和通知</li><li>屏幕捕获默认关闭，开启后会重启内核</li><li>安全模式、插件市场、内核更新和预设导入导出都可见</li>',
    'showcase.s3.caption': '真实截图 · 设置 → 扩展设置',
    'showcase.note': '截图来自真实应用构建；中英文与明暗主题均与应用状态配对，扩展设置用顶部与续页完整展示。点击任意截图可查看原图。',
  ...
  en: {
    ...
    'showcase.s3.title': 'Extensions settings for the whole desktop shell',
    'showcase.s3.body': 'The new Extensions page puts local data, LAN pairing, shortcuts, startup behavior, notifications, screen capture, and recovery in one place. Community plugins remain manual by design.',
    'showcase.s3.items': '<li>Pairing, shortcut, login, tray, and notification controls</li><li>Screen capture is off by default and restarts the kernel when toggled</li><li>Safe Mode, plugin market, kernel updates, and portable presets are visible</li>',
    'showcase.s3.caption': 'Real capture · Settings → Extensions',
    'showcase.note': 'Captured from real application builds. Chinese/English and light/dark screenshots are paired with the app state; Extensions settings are shown in a top capture and a continuation capture. Click any screenshot to open the original.',
