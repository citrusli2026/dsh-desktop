...
export var I18N = {
  zh: {
    ...
    'showcase.s3.body': '设置页首屏按日常使用频率排列：状态与数据先看，配对、快捷键和通知直接可用；平台不支持的启动能力不再伪装成灰色选项，低频工具收进“更多扩展工具”。',
    'showcase.s3.items': '<li>首屏看到状态、数据目录和手动插件边界</li><li>配对、快捷键和通知直接可用；开机启动仅在支持的 macOS / Windows 安装包显示</li><li>Safe Mode、屏幕捕获、插件市场、内核和预设进入二级区域</li>',
    'showcase.s3.caption': '真实截图 · 设置 → 扩展设置（核心）',
    'showcase.note': '截图来自真实应用构建；中英文与明暗主题均与应用状态配对，扩展设置只展示收敛后的核心页，低频工具放在应用内二级区域。点击截图可查看原图。',
  ...
  en: {
    ...
    'showcase.s3.body': 'The first view follows daily-use priority: status and data first, pairing, shortcuts, and notifications close at hand. Unsupported startup controls are omitted instead of shown as inert options; less frequent tools live under “More shell tools”.',
    'showcase.s3.items': '<li>Status, data directory, and the manual plugin boundary at a glance</li><li>Pairing, shortcut, and notification controls up front; startup controls appear only in supported packaged macOS / Windows builds</li><li>Safe Mode, screen capture, plugin market, kernel, and presets behind a secondary section</li>',
    'showcase.s3.caption': 'Real capture · Settings → Extensions (core)',
    'showcase.note': 'Captured from real application builds. Chinese/English and light/dark screenshots are paired with the app state; the Extensions card shows the focused core view, with less frequent tools tucked behind the app’s secondary section. Click any screenshot to open the original.',
