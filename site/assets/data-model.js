...
export var I18N = {
  zh: {
    ...
    'wf.s2.title': '本地且隔离', 'wf.s2.body': '只监听 loopback，默认使用独立的 ~/.dsh-desktop。',
    ...
    'wf.keeps': '<li>Agent、会话、工具与模型</li><li>官方 WebUI 与插件行为</li><li>账户配置；不新增工作台</li>',
    'ext.marker': '扩展功能',
    ...
    'latest.items': '<li><b>首次成功：</b>主界面四步引导在首条任务完成后自动结束。</li><li><b>一键体检：</b>本地检查运行时、目录、loopback、Profile 与插件状态。</li><li><b>明确边界：</b>联网检查需主动勾选，社区插件仍完全可选。</li>',
    ...
    'mk.note': '安装边界：按需在「设置 → 桌面设置 → 插件市场」手动安装 dsh-market，再到 Harness「设置 → 插件市场」手动选择其他插件；首次任务不依赖插件市场。',
    'theme.toggle': '切换明暗主题',
  ...
  en: {
    ...
    'wf.keeps': '<li>Agents, sessions, tools, and models</li><li>Official WebUI and plugin behavior</li><li>Account configuration; no second workbench</li>',
    ...
    'latest.items': '<li><b>First success:</b> a four-step main-screen guide retires after the first completed task.</li><li><b>One-click health:</b> inspect runtime, folders, loopback, profile, and plugins locally.</li><li><b>Clear boundary:</b> network probes are opt-in and community plugins stay optional.</li>',
    ...
    'ft.p1': '<h3>No Node.js install required</h3><p>Node.js and the pinned dependency set are bundled; install and launch.</p>',
    ...
    'ft.p4': '<h3>Continuous supervision and reliable updates</h3><p>Crash restarts use a budget, the tray stays available, and failed kernel updates roll back.</p>',
    ...
    'mk.note': 'Boundary: when needed, manually install dsh-market from Settings → Desktop settings → Plugin market, then choose other plugins from Harness Settings → Plugin Market. The first task never depends on the market.',
