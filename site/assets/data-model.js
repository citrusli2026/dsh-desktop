...
export var I18N = {
  ...
  en: {
    ...
    'notify.steps': '<li>System notices appear while the window is unfocused or hidden in the tray</li><li>Click a notice to show and focus the dsh-desktop window</li><li>Turn them off any time in Settings → Extension settings; preferences stay on this device</li>',
    ...
    'ft.p8': '<h3>Your desktop habits, your choice</h3><p>In Settings → Extension settings, record a summon shortcut, choose whether to launch at login and start hidden, and control desktop status notices. Preferences stay on this device; notices appear only while the window is unfocused.</p>',
    ...
    'faq.a11': 'Yes. In dsh-desktop, open Settings → Extension settings, choose Change shortcut, and press a combination with a modifier such as <code>Ctrl + Alt + K</code>. If another app already owns it, the setting reports a conflict. Launch at login is off by default; Windows and macOS can also start hidden in the tray. Linux does not currently expose a launch-at-login switch.',
    'faq.q12': 'Do desktop notices read my screen or upload data?',
    'faq.a12': 'No. Notices use only Harness public session and background-job state. They do not read screenshots, perform visual recognition, or upload data. They appear only while the window is unfocused and can be disabled in Settings → Extension settings.',
