...
export var I18N = {
  zh: {
    ...
    'showcase.lead': '这里放的都是实际运行界面，不是概念图。',
    'showcase.auto': '每 6 秒切换 · 悬停暂停', 'showcase.themes': '浅色 / 深色',
    ...
    'showcase.s3.title': '扩展设置，把关键桌面入口放在首屏',
    'showcase.s3.body': '设置页只放常用入口和可靠性恢复；平台不可用的选项不显示，低频工具收进“更多扩展工具”。',
    'showcase.s3.items': '<li>配对、快捷键和通知直接可用</li><li>Safe Mode 作为可靠性入口直接可用</li><li>屏幕捕获、插件市场、内核和预设进入二级区域</li>',
    'showcase.s3.caption': '真实截图 · 设置 → 扩展设置（核心）',
    'showcase.note': '真实应用构建 · 中英文与明暗主题配对 · 点击查看原图。',
    ...
    'ext.body': '从扩展入口启动局域网 Web 代理，手机与电脑同网后扫码即可进入 Harness。',
    'ext.steps': '<li>扩展 → 连接手机 / 平板</li><li>同一局域网扫码，确认配对码</li>',
    ...
    'latest.items': '<li><b>清晰状态：</b>桌面入口、托盘和设置保持一致。</li><li><b>直接恢复：</b>重试、Safe Mode、日志和诊断都有入口。</li><li><b>明确边界：</b>社区插件不预装，内核更新可回滚。</li>',
    ...
    'ft.p3': '<h3>原生菜单与 Harness 保持同一种语言</h3><p>应用菜单、托盘和恢复页跟随 Harness 的语言与主题设置。</p>',
    ...
    'ft.p8': '<h3>桌面习惯由你决定</h3><p>快捷键、开机启动、启动后隐藏和通知都可在扩展设置中调整。</p>',
    'nav.market': '插件市场',
    'mk.marker': '社区插件', 'mk.title': '数千个社区插件，全部由你选择',
    'mk.lead': 'dsh-desktop 只提供清晰入口和故障恢复支持；社区插件不属于安装包，也不会在首次启动时自动出现。',
    'mk.c1': '<h3>全部按需安装</h3><p>安装包不内置社区插件；先手动安装 dsh-market，再由你选择其他插件和主题。</p>',
    'mk.c2': '<h3>浏览与一键安装</h3><p>在 Harness「设置 → 插件市场」按分类浏览、安装和更新，来源可查。</p>',
    'mk.c3': '<h3>装坏了也不怕</h3><p>构建脚本先征求授权；出现问题时，用安全模式隔离第三方插件。</p>',
  ...
  en: {
    ...
    'showcase.lead': 'Real application captures, not concept mockups.',
    'showcase.auto': 'Switches every 6 seconds · pauses on hover', 'showcase.themes': 'LIGHT / DARK',
    ...
    'showcase.s3.title': 'Extensions settings, focused on the desktop essentials',
    'showcase.s3.body': 'The first view keeps useful entry points and recovery close; unavailable options are omitted and lower-frequency tools stay under “More shell tools”.',
    'showcase.s3.items': '<li>Pairing, shortcut, and notifications are ready to use</li><li>Safe Mode is a first-class recovery action</li><li>Screen capture, plugin market, kernel, and presets stay secondary</li>',
    'showcase.s3.caption': 'Real capture · Settings → Extensions (core)',
    'showcase.note': 'Real builds · Chinese/English · light/dark pairs · click to view the original.',
    ...
    'ext.body': 'Start a LAN Web proxy from Extensions. Put the phone and computer on the same network, then scan to enter Harness.',
    'ext.steps': '<li>Extensions → connect a phone or tablet</li><li>Scan on the same LAN and confirm the code</li>',
    ...
    'latest.items': '<li><b>Clear state:</b> desktop entry, tray, and settings agree.</li><li><b>Direct recovery:</b> retry, Safe Mode, logs, and diagnostics stay close.</li><li><b>Clear boundary:</b> no community plugins are bundled, and kernel updates can roll back.</li>',
    ...
    'ft.p3': '<h3>Native chrome in Harness language</h3><p>The app menu, tray, and recovery pages follow Harness language and theme settings.</p>',
    ...
    'ft.p8': '<h3>Your desktop habits, your choice</h3><p>Adjust the shortcut, launch-at-login, start-hidden, and notices in Extensions settings.</p>',
    ...
    'mk.c1': '<h3>Install only what you choose</h3><p>Nothing is bundled or auto-installed: manually install dsh-market first, then choose plugins and themes.</p>',
    'mk.c2': '<h3>Browse and one-click install</h3><p>Use Harness Settings → Plugin Market to browse, install, and update from a checkable catalog.</p>',
    'mk.c3': '<h3>Safe to experiment</h3><p>Build scripts ask first; Safe Mode quarantines third-party plugins if something breaks.</p>',
