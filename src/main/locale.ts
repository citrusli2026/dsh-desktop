...
const english = {
  ...
  'page.pluginsHeading': 'PLUGIN RECOVERY',
  'page.pluginsHint': 'These plugins failed to load, usually because they predate the bundled Harness kernel. Update them to the latest version, or disable one and start again. Disabling keeps the files and can be undone from Settings → Plugins.',
  'page.pluginsHintKernel': 'These plugins were built for an older Harness kernel and import APIs the bundled kernel no longer exports (the log shows the missing export). Updating to the latest published version almost always fixes it — or disable one and start again. Disabling keeps the files and can be undone from Settings → Plugins.',
  'page.runtimeHint': 'The bundled Node runtime failed to start (the log shows a spawn error). Its files are damaged or missing — usually an interrupted upgrade or antivirus interference. Reinstall from the full installer; your sessions and settings are kept.',
  'page.nativeModuleHint': 'A native component failed to load: usually a missing Microsoft VC++ redistributable, or antivirus quarantined a file. Install the VC++ 2015-2022 redistributable (x64, from Microsoft) and restart; if the health check reports closure integrity problems, follow its reinstall steps.',
  'page.pluginUpdate': 'Update',
  'page.pluginDisable': 'Disable',
  'page.pluginUpdateAll': 'Update all and restart',
  'page.pluginWorking': 'Working…',
  'page.pluginUpdated': 'Done — restarting…',
  'page.pluginActionFailed': 'Failed — see the logs below or export a diagnostic report.',
  'page.pluginAlreadyCurrent': 'Already the newest obtainable version — restarting…',
  'page.getHelp': 'Open a GitHub issue',
  ...
  'window.vcRuntimeTitle': 'Windows runtime library missing',
  'window.vcRuntimeMessage': 'This machine is missing the Microsoft VC++ 2015-2022 redistributable; native components (such as image processing) will not work.',
  'window.vcRuntimeDetail': 'Missing: {dlls}. Click "Open download page" to install the official VC++ redistributable (x64), then restart the app. You can also check again later via the health check.',
  'window.vcRuntimeDownload': 'Open download page',
  'window.vcRuntimeDismiss': 'Later',
...
const chinese: Record<CopyKey, string> = {
  ...
  'page.pluginsHeading': '插件恢复',
  'page.pluginsHint': '以下插件加载失败,通常是版本早于当前内置内核。可先升级到最新版,或禁用后再启动。禁用只从启动列表移除,不删除文件,之后可在「设置 → 插件」中重新启用。',
  'page.pluginsHintKernel': '这些插件是为旧版内核编译的,引用了当前内置内核已移除的导出(日志中可见缺失的导出名)。升级到最新发布版本通常即可修复——或先禁用其中一个再启动。禁用只从启动列表移除,不删除文件,之后可在「设置 → 插件」中重新启用。',
  'page.runtimeHint': '内置 Node 运行时启动失败(日志中可见 spawn 错误)。它的文件已损坏或缺失——常见于升级中断或安全软件干扰。请用完整安装包重新安装;你的会话与设置都会保留。',
  'page.nativeModuleHint': '检测到原生组件加载失败:通常是缺少 Microsoft VC++ 运行库,或杀毒软件隔离了文件。请安装 VC++ 2015-2022 运行库(x64,微软官网可下载)后重启;若运行体检报闭包文件异常,按指引重装。',
  'page.pluginUpdate': '升级',
  'page.pluginDisable': '禁用',
  'page.pluginUpdateAll': '全部升级并重启',
  'page.pluginWorking': '处理中…',
  'page.pluginUpdated': '已完成,正在重启…',
  'page.pluginActionFailed': '操作失败,请查看下方日志或导出诊断报告。',
  'page.pluginAlreadyCurrent': '已是当前可获得的最新版本,正在重启…',
  'page.getHelp': '到 GitHub 提交问题',
  ...
  'window.vcRuntimeTitle': '缺少 Windows 运行库',
  'window.vcRuntimeMessage': '检测到本机缺少 Microsoft VC++ 2015-2022 运行库,部分原生组件(如图片处理)将无法工作。',
  'window.vcRuntimeDetail': '缺少组件:{dlls}。点击「打开下载页」安装官方 VC++ 运行库(x64)后重启应用即可;也可以稍后在「运行体检」中再次查看。',
  'window.vcRuntimeDownload': '打开下载页',
  'window.vcRuntimeDismiss': '稍后再说',
