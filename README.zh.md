...
- **随时唤回工作台** —— 窗口收进托盘后，在任意应用中按可自定义的唤起快捷键即可显示并聚焦 dsh-desktop，默认是 `Ctrl/Cmd + Shift + Space`（Windows/Linux 用 Ctrl，macOS 用 ⌘）。若快捷键被其他软件占用，托盘和右键入口仍然可用。
- **按你的桌面习惯工作** —— 在 Harness「设置 → 通用」中录入快捷键，选择是否开机启动、启动后隐藏，以及是否接收本地状态通知；配置保存在壳自己的数据文件中。
- **渲染层收敛** —— 保持沙箱与上下文隔离、关闭 Node 集成、限制导航;摄像头、定位、Web 通知、文件系统权限默认拒绝(决策 0014)。可选桌面通知只使用 Harness 公开状态，不读取屏幕；点击通知即可聚焦回工作台。
