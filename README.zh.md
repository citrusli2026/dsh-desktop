...
- **坏插件也能恢复** —— 安全模式（错误页「以安全模式启动」、「桌面工具」或「桌面设置」）下第三方插件被隔离，官方与内置扩展照常运行；导出的诊断报告会列出插件清单并标出疑似肇事者。
...
- **插件坏了，Harness 起不来怎么办？** 在错误页点「以安全模式启动」（或从「桌面工具」/「桌面设置」进入安全模式）。第三方插件会被隔离，官方 bundle 与内置扩展照常运行；导出的诊断报告会列出插件清单并标出疑似肇事者，官方「设置 → 插件」仍可用于卸载它。修复后点击「退出安全模式」即可。
