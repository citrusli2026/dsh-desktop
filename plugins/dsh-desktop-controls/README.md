...
The first view stays focused on useful desktop actions: LAN pairing, the summon
shortcut, notifications, startup behavior, and Safe Mode recovery. Runtime status
belongs in the extension panel and tray, not in a settings page. Lower-frequency
controls — screen capture, the plugin market, kernel actions, and portable presets —
live under the collapsed "More shell tools" section. Platform-dependent controls are
rendered only when usable: launch-at-login is available in packaged macOS/Windows
builds, start-hidden appears only after launch-at-login is enabled, and unavailable
desktop notifications are omitted instead of shown as disabled controls.
