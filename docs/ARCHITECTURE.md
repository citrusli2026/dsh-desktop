...
最后更新: 2026-09-24 · 当前代码基线 `0.1.7-rc.1.shell.2`（未发布）
（主界面提供一次性首次成功引导；桌面设置提供本地优先、只读且脱敏的运行体检；
发布门禁覆盖真实 Harness、插件市场、安全模式、移动设备连接和跨版本升级；内核 `0.1.7-rc.1`）
...
原生菜单、托盘、窗口标题与通知的语言及主题由 `ShellLocaleController` 管理，优先
读取 `profiles/web/cordis.patch.yml` 中 `locale`、`ui-theme` 的 `config.preference`；
仍兼容旧版 `settings.yaml` 和内核迁移归档 `settings.yaml.imported`，并监听 profile
更新。只有 profile、现存设置和旧文件归档都没有语言偏好时才使用系统语言；归档存在时
不会重新创建旧设置文件。
