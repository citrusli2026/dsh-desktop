...
## 7c. 流行 DSH 桌面 Windows 版签名逆向实测(2026-09-18)

下载 5 个最流行同类项目的 Windows 安装器逐一解剖 Authenticode(工具:signify 0.9.2;样本 = 各自 Latest release 的 x64 exe):

| 项目(星数) | 签名 | 证书事实 |
|---|---|---|
| anywhere-labs(21,443★,全场最高) | **❌ 未签名** | 2 万星也没解决——个人/未签名交付在头部项目同样存在 |
| dataelement(3,023★) | ✅ **GlobalSign OV** | 主体 `Beijing Shuju Xiangsu Intelligence Technology Co., Ltd.`(中国公司,营业执照主体);有效期 2026-08-24 → 2027-08-25(**一个月前才办的,一年期**);RFC3161 时间戳(DigiCert);私钥在其自托管 runner 的 SafeNet UKey |
| cocode(161★) | ✅ **Sectigo EV** | 主体 `广东灯塔引擎科技有限公司`(subject 含 `businessCategory=Private Organization` + serialNumber——CA/B EV 规范字段);3 年期(2025→2028,老规则下签发);RFC3161 |
| myYangyunfan(597★) | ❌ 未签名 | — |
| vibeinging(632★) | ❌ 未签名 | — |

**三个直接结论**:
1. **签了名的两个都是中国公司拿营业执照走的 GlobalSign OV / Sectigo EV**——公司路线与个人无关,但证明"中国主体 + 国际 CA + CI 后置签名 + RFC3161"这条链完全跑得通。
2. **5 个流行版里 3 个未签名,包括 21k 星的头部项目**——个人开发者没有护照/营业执照时,整个社区尚无人解决;我们如果用 SignPath 签上,交付品质反而领先全场。
3. dataelement 的证书 2026-08-24 刚签发——同行也在近期才补签名,我们不算晚。

**因"无护照"修订路线**(原 Certum 需拉丁字符证件 = 护照,出局):

| 顺位 | 路线 | 无护照可行性 |
|---|---|---|
| 1(升为唯一主攻) | **SignPath Foundation** | ✅ **完全不验证个人身份证件**——它验证"二进制由公开源码构建"(构建来源),不是"你是谁"。护照缺失零影响;MIT 内核资格问题已缓解(§7b);剩余确认项只有捆绑认定与单人三角色 |
| 2(改问法) | **SSL.com IV** | 发客服问:**中国大陆居民身份证**是否在 IV 验证接受的证件列表(此前只核实过护照/驾照;SSL.com 有中文支持,中国开发者持有案例多)。若接受 → ≈¥2,082/年备胎复活 |
| 3(降级) | Certum | 除非 support 接受无护照材料(可能性低),放弃 |
| 并行 | Microsoft Store | 证件 + 自拍验证——**身份证大概率可用**(待确认大陆在市场列表);中期渠道权重上调 |

维护者动作修订:原"三动作"改为 **1) 发 SignPath 申请(唯一无证件要求的路线,先行);2) 问 SSL.com 身份证问题;3) 等 SignPath 答复期间维持不签名 + 补 README 未签名提示**。
