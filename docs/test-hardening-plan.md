...
| 已知边界 | 记录 | 真实对话闭环（API key）、OS 版本矩阵声明对齐 — 保持文档记录，不做；跨版本升级已由 alpha.4.shell.3 的三平台发布门禁覆盖 |

## 10. 手机连接与插件恢复现场门禁（2026-09-07）

本轮把容易停留在“需要人工点一下”的两条路径收成可重复验证：

| 路径 | 自动化证据 | 仍需物理设备 |
|---|---|---|
| 桌面二维码 → 手机配对 | Playwright 从二维码窗口读取实际 LAN 地址和一次性配对码，再由 Android 15 模拟器中的真实 Chrome 打开深链并提交配对；同时保留移动 Chromium/WebKit 浏览器覆盖 | 相机对屏幕二维码的光学识别、不同厂商相机/浏览器的跳转策略 |
| 手机会话续用 | 模拟器 Chrome 完成后台/恢复、停止并重新开启共享、桌面应用完整退出并重启，逐段校验 `/session/check` 与页面重新加载 | Wi-Fi AP 隔离、蜂窝/无线切换、锁屏省电策略等真实网络变量 |
| 坏插件恢复 | 开发态验证崩溃插件自动隔离；真实市场 E2E 验证崩溃 → 隔离 → 升级并重新启用 → Harness 重启成功；打包 Safe Mode 验证故障隔离 | 第三方私有 registry、企业代理和需要交互式认证的安装脚本 |

Android 门禁默认跳过，避免没有 SDK/AVD 的 CI 误报；本机启动模拟器后执行：

```sh
DSH_E2E_ANDROID_SERIAL=emulator-5554 pnpm run test:e2e:android
```

该测试使用 Chrome DevTools 协议做确定性页面控制，但截图来自 `adb screencap`，因此
验证对象是模拟器内真实 Chrome 进程，而不是 Playwright 的移动设备外观模拟。二维码
SVG 内容和深链生成由桌面 E2E 覆盖；相机光学扫码明确留给物理设备检查，不把深链注入
表述为已经验证相机。
