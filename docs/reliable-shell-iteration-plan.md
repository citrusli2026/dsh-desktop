...
### 6.7 alpha.4.shell.3：插件安装与升级可靠性闭环（本轮）
...
>
> **交付状态（2026-09-15，随 v0.1.5-rc.2.shell.14 发布）**：X1–X4 全部落地——体检修复闭环（repairUrl 双线路 + 三步指引 + 诊断直链）、Defender 探测与 S4 条件轮落地；X3 去护卫观察一轮后回退（CI Linux worker teardown 挂起复现，退役搁置至根因消除）；X4 共享基线被 NSIS 卸载器内嵌目录击败、回退逐场景真实安装（发现录入分析文档）。门禁：282 单测、dev E2E 16/16、LAN 2/2、offline+real market、closure smoke、SAFE_BREAK 本地全绿。
...
| X4 | 残留矩阵降本 | 尝试共享 previous 基线（目录树复制），被 NSIS 卸载器编译期内嵌安装目录击败（复制树的卸载器删源目录）；回退逐场景真实安装，发现录入分析文档；Defender 探测与 S4 条件轮保留 | `scripts/smoke-nsis-residue.mjs` | Windows job 维持 ~29.5 分钟（正确性优先）；降本留待并行 runner 等手段 |

运维项（随轮次走，不单独立项）：#39/#33 巡检闭环继续；签名决策待用户启动（§7）。

### 6.13 rc.2.shell.1：垃圾桶完善轮（2026-09-18 完成）

用户反馈"垃圾桶 UI 丑、与其他地方风格不一样"后分析立项。分析结论：主进程 28 项单测扎实但 UI 零测试、零设计体系接入。交付（除 P1-③ 调整为保留枚举外全部落地）：

| 项 | 交付 | 验收 |
|---|---|---|
| P0-① 设计体系统一 | TrashSection 重写：分组卡片/真 tablist/类型徽标/保留期倒计时/引导空态 | 与桌面设置区块同构；契约断言锁定 |
| P0-② 危险动作确认 | 彻底删除/清除已过期行内两步确认；会话删除正名「删除（入桶）」 | 确认文案可见后才可执行 |
| P1-③ plugin 生产者 | **调整**：壳不拥有插件卸载（内核 WebUI 负责），枚举保留不造生产者 | 文档如实记录 |
| P1-④ kernel 生产者 | `retireKernelOverlay`：恢复内置后 overlay 入桶（30 天窗口，失败保持原装） | 3 项单测 + 接线断言 |
| P1-⑤ 会话页签分组 | 已归档/未归档分组 + 模型说明文案；删除按钮正名 | E2E 断言分组标题 |
| P2-⑥ UI E2E | `e2e/trash-ui.spec.ts` 打包全链（还原/启动期过期清理/确认清除/会话入桶） | 1/1 通过 |
| P2-⑦ 契约断言 | 设计体系 class、确认交互、tablist、内核接线 | contract test 5/5 |
| 意外收获 | 修复归档判定失效（内核 `global` 嵌套未兼容）——E2E 夹具暴露的既有缺陷 | 归档/取消归档按内核真实文件结构工作 |
