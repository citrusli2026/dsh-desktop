...
Versions and tags are composite: `<dsh version>.shell.<shell rev>` — e.g. `0.1.7-alpha.1.shell.0` bundles `@deepseek-ai/dsh` 0.1.7-alpha.1 at shell revision 0. `scripts/version.mjs` owns the version field; a daily `dsh-watch` workflow checks upstream npm and opens a verified bump PR automatically.
