...
Versions and tags are composite: `<dsh version>.shell.<shell rev>` — e.g. `0.1.5-rc.2.shell.5` bundles `@deepseek-ai/dsh` 0.1.5-rc.2 at shell revision 5. `scripts/version.mjs` owns the version field; a daily `dsh-watch` workflow checks upstream npm and opens a verified bump PR automatically.
