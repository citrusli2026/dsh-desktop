...
Version and tag are composite: `<dsh version>.shell.<shell rev>` — e.g. `0.1.0-rc.8.shell.3` bundles `@deepseek-ai/dsh` 0.1.0-rc.8 at shell revision 3. `scripts/version.mjs` owns the version field (`show` / `check` / `bump`); a daily `dsh-watch` workflow checks upstream npm and opens a verified bump PR automatically (decision 0009).
